...
const Professionals = () => {
  ...
  const fetchProfessionals = async () => {
    try {
      ...
      setProfessionals(
        Array.isArray(data.professionals) ? data.professionals : Array.isArray(data) ? data : []
      );
    } catch (error) {
    ...
    <div className="p-4 w-full">
      ...
      <div className="relative flex flex-col w-full h-full text-gray-700 bg-white shadow-md rounded-lg bg-clip-border overflow-hidden">
        {isLoading ? (
          ...
          <div className="text-center p-4 text-gray-500">No professionals found</div>
        ) : (
                ...
                <div className="mb-4">
                  ...
                  <Calendar
                    ...
                    value={
                      formData.notAvailable.length > 0
                        ? [
                            new Date(formData.notAvailable[0].from),
                            new Date(formData.notAvailable[0].to),
                          ]
                        : null
                    }
                  ...
                  {formData.notAvailable.length > 0 && (
                    <div className="mt-2 text-sm text-gray-700">
                      Selected Range: {new Date(
                        formData.notAvailable[0].from
                      ).toLocaleDateString()} - {new Date(
                        formData.notAvailable[0].to
                      ).toLocaleDateString()}
                    </div>
                  )}
                </div>
